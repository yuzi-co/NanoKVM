...
// and never starting with a dot or a dash. The character class holds no path
// separator, so a name that matches is always its own base name.
...
// below dir, after resolving any "." and ".." segments. Both arguments must be
// absolute; a relative one never reports as contained.
