...
func newJiggler(t *testing.T) *Jiggler {
	...
	t.Cleanup(func() { stopJiggler(j) })

	return j
}

// stopJiggler asks the loop to finish so a test's goroutine does not outlive
// it. It returns before the loop has noticed. This lives here rather than on
// Jiggler because nothing in the server ever stops the loop without also
// clearing the config file, which Disable already does.
func stopJiggler(j *Jiggler) {
	j.mutex.Lock()
