...
func TestProcessRejectsAboveNyquist(t *testing.T) {
	...
	if p := peak(got); p > 500 {
		t.Errorf("6 kHz tone came through at peak %.0f, want under 500 of 10000", p)
	}
}
...
const (
	...
	// filterTaps and cutoffHz place the passband edge at 3.4 kHz and the
	// stopband from 4 kHz, which is Nyquist for the 8 kHz output. 49 taps is
	// enough for that transition and costs 49 multiplies per output sample,
	// which is about 400 thousand per second.
	filterTaps = 49
	cutoffHz   = 3600.0
)
...
If `TestProcessRejectsAboveNyquist` fails by a small margin, raise `filterTaps` to 65 rather than lowering `cutoffHz`: a lower cutoff dulls speech, more taps sharpen the transition.
...
**Interfaces:**
- Consumes: `audio.NewStream`, `audio.Available`, `Client.enqueueAudio`, `Client.writeAudio`.
- Produces: `func (m *WebRTCManager) StartAudioStream()`, `func (m *WebRTCManager) stopAudioStreamIfIdle()`.
...
// StartAudioStream begins capture if a client is connected and the gadget has
// a capture card. Availability is checked here, not cached at start, because
// the settings switch rebuilds the gadget while the server runs.
...
In `AddClient`, start the audio writer beside the video one:
...
and at the top level of the translation object, beside the other menu sections, add:
