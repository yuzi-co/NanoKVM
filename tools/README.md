...
The supervisor is the only component that outlives the server, so it is the
only place that can record what the carveout held at the moment a restart
happens. It writes one line to `SUPERVISE_LOG` at the start of every restart,
...
carveout erodes with restarts, not with uptime: a dead process keeps its whole
ION working set, because the buffers belong to the kernel driver rather than
to the process, so `gen` above 1 means an earlier restart already orphaned a
generation that nothing has freed.
...
file blocks forever in uninterruptible sleep, and a supervisor that could wedge
would defeat the escalation feature it carries. A board without the debugfs
