...
func TestProcessDownmixesToMono(t *testing.T) {
	...
	//
	// The negative sample goes through a variable. Converting a negative
	// constant straight to uint16 is a compile error, because both conversions
	// would be constant expressions and uint16 cannot represent -8000. Through
	// a variable the conversion happens at run time and reinterprets the bits,
	// which is what the little-endian parse in Process reverses.
	left := int16(8000)
	right := int16(-8000)
...
Expected: the `uac1.usb0` line is the **last** `ln -s functions` in the file, after the three
`hid.GS*` links and after `mass_storage.disk0`.

Do not expect to find `acm.GS0` here. This branch is cut from `main`, and the ACM block lives on
`fix/usb-acm-console`. The device's own `/etc/init.d/S03usbdev` does carry one, which is a different
file from the repository's copy — see [[init-scripts-live-in-two-places]]. If the observed output
disagrees with this expectation, the disagreement is the finding.
...
The happy path is the first group. The groups after it are the lifecycle cases, and each one is
there because a defect hides in it and nowhere else.
...
### A restart while somebody listens

An orphaned `arecord` holds `hw:UAC1Gadget,0` exclusively. The next server cannot record, it spends
its five restarts in about three seconds, and audio stays dead until an operator logs in. The child
does not exit by itself: it sees the closed pipe only when it writes, and while the host plays
nothing it blocks in the ALSA read. A restart with no viewer connected does not show this, because
no child is running.

- [ ] Start playback and unmute in the browser, so `ps | grep arecord` shows one child.
- [ ] Run `/etc/init.d/S95nanokvm restart` while that viewer still listens.
- [ ] Run `ps | grep arecord`. Expect no `arecord`, or exactly one that the new server started.
- [ ] Reconnect the browser and confirm the sound comes back without any manual step.
- [ ] Repeat the restart three times in a row. Expect the same result each time, not a count that grows.
- [ ] Repeat once with `killall -9 NanoKVM-Server` instead of the restart, then start the service. The server reaches no code on that path, so this checks the `killall arecord` in `S95nanokvm` rather than `dispose()`.

### The switch thrown under a live listener

The gadget teardown removes the card underneath a blocked `snd_pcm_readi`. This is the case the
bounded wait in `Stream.Stop` exists for.

- [ ] With a viewer listening to real playback, turn **Virtual Speaker** off in `Settings > Device`.
- [ ] Expect the browser to go quiet within a second or two, with no error dialog.
- [ ] Expect the video stream to keep running at its normal frame rate.
- [ ] Expect HID to survive: type into the host and move the mouse. Do not write to `/dev/hidg*` to test this.
- [ ] Confirm the server did not wedge: the web UI still answers, and `/api/hid/status` still responds.
- [ ] Check the log for `audio capture did not stop in 2s`. That line means the child wedged and the server walked away from it, which is the designed behaviour rather than a fault, but it is worth knowing that it happened.
- [ ] Turn the switch back on, reconnect the browser, and confirm audio returns.

### Two viewers, one from before the switch

A client that connects while the gadget has no card negotiates no audio track and never gets one.
Capture must stop when the last *listener* leaves, not when the last *client* leaves.

- [ ] With **Virtual Speaker** off, connect viewer A and leave it on the WebRTC screen. Confirm A shows no speaker button.
- [ ] Turn **Virtual Speaker** on. Confirm A still shows no speaker button, because its tracks were settled at negotiation.
- [ ] Connect viewer B. Confirm B shows the speaker button and hears the host.
- [ ] Close viewer B while A stays connected.
- [ ] Run `ps | grep arecord`. Expect no child. A child still running here is the defect.
- [ ] Reload viewer A. Confirm it now shows the speaker button and hears the host.

### Cost on one core

The design claims about one million multiply-accumulate operations a second for the FIR, and that
this is small next to what the video pipeline already costs. Nothing off the board can check it.

- [ ] Run `top` on the device with no viewer connected and record the idle figures.
- [ ] Connect one viewer, unmute, and play continuous sound on the host. Run `top` again.
- [ ] Record the CPU percentage of `NanoKVM-Server` and of `arecord`, and their RSS. The design expects `arecord` near 1232 kB.
- [ ] Compare the video frame rate before and after, from `cat /kvmapp/kvm/now_fps`. Expect no measurable drop.
- [ ] Repeat with two listeners. The FIR runs once for everyone, so the second listener must cost packetization and writes only.
- [ ] Do not read `/proc/cvitek/vb` during any of this. It blocks forever in D state, and the load average on this board is not a CPU signal.

### A device with audio off

This is what almost every device looks like, because the marker file is opt-in.

- [ ] On a device with no `/boot/usb.uac`, open the WebRTC screen.
- [ ] Confirm the menu bar shows **no** speaker button at all.
- [ ] Confirm `Settings > Appearance` still lists **Speaker** as a hideable icon, and that turning it off and on changes nothing on the menu bar while audio is unavailable.
- [ ] Confirm the browser console shows no error from the audio transceiver, and that video is unaffected.

### Both host operating systems

Windows and Linux differ in how they treat a newly attached USB sound device, and the switch
attaches one while the host is running.

- [ ] Windows host: throw the switch and watch whether Windows moves the default output to NanoKVM by itself. Record which it does.
- [ ] Windows host: play a sound and confirm it reaches the browser. Confirm the host's own speakers behave as the operator expects afterwards.
- [ ] Linux host: repeat both steps. PulseAudio and PipeWire both tend to switch on a new sink; record what happened.
- [ ] On both hosts, confirm that turning the switch off returns the default output to what it was, or note that it does not, because the operator has to be told.
- [ ] On both hosts, confirm the keyboard and the mouse keep working across the switch.
